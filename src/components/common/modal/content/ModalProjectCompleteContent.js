...
const ModalProjectCompleteContentState = {
  checked: null,
};
function ModalProjectCompleteContent(props) {
  const { onChange = () => {}, defaultValue = 1, value = 1 } = props;
  const [values, setValues] = useImmer(ModalProjectCompleteContentState);
  const completeIndex = 1;
  const onlyUploadIndex = 0;
  ...
  return (
    <Styled.ModalProjectCompleteContent>
      <p className="ProjectDeleteModal__info tx">
        When the upload is complete, the partner can download the file.
      </p>
      <RadioGroup
        aria-label="gender"
        name="gender1"
      ...
      >
        <FormControlLabel
          value={onlyUploadIndex}
          control={<Radio className="hiddenRadio " />}
          className="ProjectDeleteModal__labelContol"
          label={
            <span className={cx('ProjectDeleteModal__label', { on: checkVal === onlyUploadIndex })}>
              Upload Only
            </span>
          }
        />
...
const Styled = {
  ModalProjectCompleteContent: styled.div`
    & {
      ${disableDrag};
...
export default ModalProjectCompleteContent;
